...
public class JdbcAccountServiceImpl extends JdbcRepositoryWrapper implements AccountService {
  ...
  private static final String CREATE_STATEMENT = "CREATE TABLE IF NOT EXISTS `account` (\n" +
  ...
  private static final String INSERT_STATEMENT = "INSERT INTO account (id, username, phone, email, birthDate) VALUES (?, ?, ?, ?, ?)";
  private static final String EXISTS_STATEMENT = "SELECT EXISTS(1) FROM account WHERE username = ?";
  private static final String FETCH_STATEMENT = "SELECT * FROM account WHERE id = ?";
  private static final String FETCH_BY_USERNAME_STATEMENT = "SELECT * FROM account WHERE username = ?";
  private static final String FETCH_ALL_STATEMENT = "SELECT * FROM account";
  private static final String UPDATE_STATEMENT = "UPDATE `account`\n" +
    "SET `username` = ?,\n" +
    "`phone` = ?,\n" +
    "`email` = ?,\n" +
    "`birthDate` = ? \n" +
    "WHERE `id` = ?";
  private static final String DELETE_STATEMENT = "DELETE FROM account WHERE id = ?";
  private static final String DELETE_ALL_STATEMENT = "DELETE FROM account";
