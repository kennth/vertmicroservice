...
public class APIGatewayVerticle extends RestAPIVerticle {
  ...
  public void start(Future<Void> future) throws Exception {
    ...
    // list service
    router.get("/api/s").handler(this::listService);
   ...
   // oauth2 = KeycloakAuth.create(vertx, OAuth2FlowType.AUTH_CODE, config());
    ...
    //srouter.route("/callback").handler(context -> authCallback(oauth2, hostURI, context));
  ...
  private void listService(RoutingContext context) {
    Future<List<Record>> future = Future.future();
    discovery.getRecords(record -> {System.out.println(record.getName());return false;},
            future.completer());
    context.response().end("try");
  }
